      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="space-y-6">
            ...
            <div className="flex space-x-4">
              {[Facebook, Twitter, Instagram, Linkedin].map((Icon, i) => (
                <a key={i} href="#" className="text-slate-400 hover:text-brand transition-colors">
                  <Icon size={20} />
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-4">
              {['Fonctionnalités', 'Tarifs', 'Télécharger', 'Guides', 'Témoignages'].map((item) => (
                <li key={item}>
                  <a href="#" className="text-slate-600 hover:text-brand transition-colors">{item}</a>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            <h3 className="font-bold text-slate-900 mb-6">Compagnie</h3>
            <ul className="space-y-4">
              {['À propos', 'Carrières', 'Presse', 'Blog', 'Contact'].map((item) => (
                <li key={item}>
                  <a href="#" className="text-slate-600 hover:text-brand transition-colors">{item}</a>
                </li>
              ))}
          ...
          <div>
            ...
            <p className="text-slate-600 mb-4">Recevez nos derniers conseils et mises à jour.</p>
            <form className="space-y-3" onSubmit={(e) => e.preventDefault()}>
              <input
                type="email"
                placeholder="Votre email"
                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-brand focus:ring-2 focus:ring-teal-100 outline-none transition-all"
              />
        ...
        <div className="border-t border-slate-200 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex space-x-6 text-sm text-slate-500">
            ...
            <a href="#" className="hover:text-slate-900">CGU</a>
